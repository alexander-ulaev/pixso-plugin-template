...
export class TailwindDefaultBuilder {
  ...
  blend(): this {
    this.addAttributes(
      ...
      tailwindRotation(this.node as any),
      tailwindOpacity(this.node as any),
      tailwindBlendMode(this.node as any),
    );
   ...
   */
  shadow(): this {
    this.addAttributes(...tailwindShadow(this.node as any));
    return this;
  }
  ...
  blur() {
    ...
    if ("effects" in node && node.effects.length > 0) {
      const blur = node.effects.find(
        (e: any) => e.type === "LAYER_BLUR" && e.visible,
      );
      if (blur) {
      ...
      const backgroundBlur = node.effects.find(
        (e: any) => e.type === "BACKGROUND_BLUR" && e.visible,
      );
      if (backgroundBlur) {
