...
export const htmlMain = async (
  ...
  try {
    htmlContent = await htmlWidgetGenerator(sceneNode, settings);
  } catch (err) {
    console.log("error htmlWidgetGenerator", err)
  }
  ...
  if (mode !== "html") {
    ...
    try {
      ...
      console.log("error generateComponentCode", err)
    }
...
const convertNode = (settings: HTMLSettings) => async (node: any) => {
  console.log("convertNode", node.name, node.type)
  ...
  switch (node.type) {
    ...
    case "ELLIPSE":
      {
        console.log("0 ELLIPSE");
        const r = await htmlContainer(node, "", [], settings);
        console.log("1 ELLIPSE", r);
        return r;
      }
    case "GROUP":
      {
        console.log("0 GROUP");
        const r = await htmlGroup(node, settings);
        console.log("1 GROUP", r);
        return r;
      }
    case "FRAME":
    case "COMPONENT":
    ...
    case "COMPONENT_SET":
      {
        console.log("0 COMPONENT_SET");
        const r = await htmlFrame(node, settings);
        console.log("1 COMPONENT_SET", r);
        return r;
      }
    case "SECTION":
      {
        console.log("0 SECTION");
        const r = await htmlSection(node, settings);
        console.log("1 SECTION", r);
        return r;
      }
    case "TEXT":
      {
        console.log("0 TEXT");
        const r = await htmlText(node, settings);
        console.log("1 TEXT", r);
        return r;
      }
    case "LINE":
    ...
    case "VECTOR":
      {
        ...
        console.log("0 VECTOR");

        const r = await htmlContainer(
          { ...node, type: "RECTANGLE" } as any,
          "",
          [],
          settings,
        );
        console.log("0 VECTOR", r);
...
const htmlText = (node: any, settings: HTMLSettings): string => {
  console.log("htmlText 1")
  let layoutBuilder = new HtmlTextBuilder(node, settings)
    .commonPositionStyles()
    .textTrim()
    .textAlignHorizontal()
    .textAlignVertical();
  console.log("htmlText 2")

  const styledHtml = layoutBuilder.getTextSegments(node);
  console.log("htmlText 3")

  previousExecutionCache.push(...styledHtml);
  console.log("htmlText 4")
...
const htmlFrame = async (
  ...
  console.log("htmlFrame", node.children)
...
const htmlContainer = async (
  ...
  console.log("htmlContainer 0")
  const builder = new HtmlDefaultBuilder(node, settings)
    .commonPositionStyles()
    .commonShapeStyles();

  console.log("builder 0", builder)

  if (builder.styles || additionalStyles) {
    let tag = "div";
    let src = "";

    console.log("nodeHasImageFill 0");
    if (nodeHasImageFill(node)) {
      console.log("nodeHasImageFill 1");
      ...
      if (
        ...
        console.log("exportNodeAsBase64PNG", altNode)
    ...
    console.log("nodeHasImageFill 2");

    const build = builder.build(additionalStyles);
    console.log("builder.build", build);
    ...
    console.log("Standard HTML approach for HTML, React, or Svelte");
  ...
  console.log("return children 0", children)
  return children;
};
...
const htmlLine = (node: any, settings: HTMLSettings): string => {
  const builder = new HtmlDefaultBuilder(node, settings)
    .commonPositionStyles()
    .commonShapeStyles();
...
export const htmlCodeGenTextStyles = (settings: HTMLSettings) => {
  ...
  console.log("previousExecutionCache", previousExecutionCache);
