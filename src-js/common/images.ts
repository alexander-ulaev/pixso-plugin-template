...
const fillIsImage = ({ type }: any) => type === "IMAGE";

export const getImageFills = (node: any): any[] => {
  try {
    return (node.fills as any[]).filter(fillIsImage);
  } catch (e) {
    return [];
  }
};

export const nodeHasImageFill = (node: any): Boolean =>
  getImageFills(node).length > 0;

export const nodeHasMultipleFills = (node: any) =>
...
export const exportNodeAsBase64PNG = async <T extends ExportableNode>(
  ...
  let bytes:any = [];
  try{
   bytes = await exportAsyncProxy(n, exportSettings);
  }catch(e){
    console.log("шляпа шляпаня", e)
  }
  if (temporarilyHideChildren) {
